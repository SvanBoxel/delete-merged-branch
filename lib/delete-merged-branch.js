...
module.exports = async (context) => {
  const config = await context.config('delete-merged-branch-config.yml', {
    exclude: [],
    delete_closed_pr: shouldClosePrByDefault(),
    included: []
  })
  ...
  const targetName = context.payload.pull_request.base.ref
  ...
  if (config.include.length > 0 && !config.include.some((rule) => new RegExp(`^${rule.split('*').join('.*')}$`).test(targetName))) {
    context.log.info(`Target branch ${targetName} excluded. Keeping ${owner}/${repo}/${ref}`)
    return
  }
